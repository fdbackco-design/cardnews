...
4. 글자 수(공백 포함, **중간 자르기 금지** — 넘으면 문장 전체를 짧게 다시 쓸 것):
   - intro: 1문장, 26자 이내
   - highlights: 1~2문장, 각 26자 이내
   - outro: 0~1문장, 26자 이내 (없으면 null)
...
원문 제목(deckTitle)은 학술적이고 길다. 사람들이 스크롤을 멈추고 다음 장이 궁금해지도록 만드는 **짧은 건강 지식형 제목**으로 완전히 새로 작성해라. 원문을 단순히 자르거나 요약하지 마라.

1. rewrittenCoverTitle:
   - coverTitleLines 두 줄을 공백으로 이어붙인 값. 총 20자 이내.
   - 핵심 키워드 + 궁금증/행동 유도 톤.
   - 원문 제목을 그대로 복사하거나 단순히 잘라쓰지 마라.

2. coverTitleLines: ⚡ 가장 중요한 규칙
   - **반드시 정확히 2줄** 배열.
   - **각 줄은 공백 포함 10자 이내** — 1자라도 초과하면 재작성.
   - 조사(및, 으로, 와, 과, 에서, 의)만 남은 채 줄을 끊지 마라.
   - 마침표 금지. 필요 시 물음표만 허용.

3. 금지 — 광고성·낚시성·의학적 단정 표현:
   - "충격", "절대", "비밀", "마법", "100%", "완치" 등 사용 금지.
   - 원문에 없는 의학적 효과·단정 표현 추가 금지.

4. 좋은 예 (원문 → 재작성):
   - "임신 중 혈압 관리, 산모와 태아를 지키는 첫걸음"
     → coverTitleLines: ["임신 중 혈압", "관리법 총정리"]  ← 7자, 7자 ✓
   - "족저근막염: 아침 첫발의 고통, 원인과 대처법"
     → coverTitleLines: ["아침 첫발", "찌릿 해법은?"]  ← 5자, 7자 ✓
   - "우리 아이 성장 로드맵, 영유아 건강검진으로 완성하세요!"
     → coverTitleLines: ["아이 잘 크나?", "검진 가이드"]  ← 7자, 6자 ✓
   - "콘택트렌즈 올바른 사용법"
     → coverTitleLines: ["렌즈 끼기 전", "꼭 볼 것"]  ← 6자, 5자 ✓

5. 나쁜 예 (이런 제목은 절대 쓰지 마라):
   - "콘택트렌즈, 올바른 사용이 눈 건강" (원문 단순 축약, 한 줄이 10자 초과)
   - "고혈압의 정의와 예방 방법" (학술적, 재미 없음)`;
...
export const CARD_DECK_RESPONSE_SCHEMA = {
  ...
  properties: {
    rewrittenCoverTitle: {
      type: "STRING",
      description:
        "원문 내용을 바탕으로 완전히 새로 쓴 카드뉴스용 표지 제목. coverTitleLines 두 줄을 이어붙인 값. 총 20자 이내. 원문 그대로 복사 금지.",
    },
    coverTitleLines: {
      type: "ARRAY",
      items: { type: "STRING" },
      description:
        "표지 제목 배열 — 반드시 정확히 2개. 각 줄 공백 포함 10자 이내(초과 시 재작성). 조사로 줄 끊지 마라. 마침표 금지.",
    },
    coverImagePrompt: {
      type: "STRING",
      description:
    ...
    cards: {
      ...
      items: {
        ...
        properties: {
          ...
          intro: { type: "STRING", description: "도입 1문장, 공백 포함 26자 이내" },
          highlights: {
            type: "ARRAY",
            items: { type: "STRING" },
            description: "핵심 실천 요령 1~2문장, 각 공백 포함 26자 이내",
          },
          outro: {
            type: "STRING",
            nullable: true,
            description: "마무리 1문장, 공백 포함 26자 이내, 또는 null",
          },
          imagePrompt: {
            type: "STRING",
            description:
...
export const TOPIC_GENERATE_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    rewrittenCoverTitle: {
      type: "STRING",
      description: "카드뉴스 표지 제목. coverTitleLines 두 줄을 이어붙인 값. 총 20자 이내. 정보성·호기심 유발형. 광고성 표현 금지.",
    },
    coverTitleLines: {
      type: "ARRAY",
      items: { type: "STRING" },
      description: "표지 제목 배열 — 반드시 정확히 2개. 각 줄 공백 포함 10자 이내(초과 시 재작성). 조사로 줄 끊지 마라. 마침표 금지.",
    },
    coverImagePrompt: {
      type: "STRING",
    ...
    cards: {
      ...
      items: {
        ...
        properties: {
          ...
          intro: {
            type: "STRING",
            description: "도입 1문장, 공백 포함 26자 이내. 해당 카드의 핵심 주제를 설명하는 완결 문장.",
          },
          highlights: {
            type: "ARRAY",
            items: { type: "STRING" },
            description: "핵심 실천/정보 문장 1~2개, 각 공백 포함 26자 이내. 카드마다 반드시 다른 내용.",
          },
          outro: {
            type: "STRING",
            nullable: true,
...
- intro: 공백 포함 26자 이내, 해당 항목이 왜·어떻게 효과적인지 1문장
- highlights: 1개 권장(최대 2개), 각 공백 포함 26자 이내, 카드마다 반드시 다른 내용
- outro: 공백 포함 26자 이내, 해당 항목의 보충 팁 / 없으면 null
...
━━━ 표지 제목 ⚡ (규칙 위반 시 자동 재시도) ━━━
- coverTitleLines: **반드시 정확히 2줄**, 각 줄 **공백 포함 10자 이내**
  - 1자라도 초과하면 재작성. 마침표 금지. 조사로 줄 끊지 마라.
  - 좋은 예: ["렌즈 끼기 전", "꼭 볼 것"]  ["혈압 높다면", "이것부터"]
  - 나쁜 예: ["콘택트렌즈, 올바른 사용이"] (10자 초과)
- rewrittenCoverTitle: coverTitleLines 두 줄을 공백으로 이어붙인 값, 총 20자 이내
- 광고성·낚시성·의학적 단정 표현 금지`;
...
- intro: 그 항목이 왜·어떻게 효과적인지 1문장. **공백 포함 26자 이내**. **주제명("${topic}은/는") 으로 시작 금지**.
- highlights: 그 항목의 구체적 방법·기준·예시 (수치·재료·시간 포함 권장) 1개. **공백 포함 26자 이내**.
- outro: 그 항목의 보충 팁 또는 시작 방법 (참고 내용 근거) — **공백 포함 26자 이내**, 없으면 null.
